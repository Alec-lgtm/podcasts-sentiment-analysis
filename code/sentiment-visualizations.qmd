...
# `{r} # #text side so that we get a wholistic view # #turns the transcript into a character vector so easier to deal with # Ptext <- Ptranscript$text #  # #cleans the text so that it is only words then makes it a dataframe # Ptext_clean <- Ptext %>% #   str_to_lower() %>% #   str_remove_all("[^[:alpha:][:space:]]") %>% #   data.frame(text = .) #  # #splits the individual words for sentiment usage # Ptext_split <- Ptext_clean %>% #   unnest_tokens(word, text) %>% #   anti_join(stop_words, by = "word") #  #  # ##Articles # Atext <- VoxA_transcript$text #  # Atext_clean <- Atext %>% #   str_to_lower() %>% #   str_remove_all("[^[:alpha:][:space:]]") %>% #   data.frame(text = .) #  # Atext_split <- Atext_clean %>% #   unnest_tokens(word, text) %>% #   anti_join(stop_words, by = "word") #  #`
