...
transcript_cleaned$date <- as.Date(transcript_cleaned$date,format = "%m_%d_%y")
...
VoxA <- readRDS("..\\data\\test_scraped_pages.rds")
VoxA_transcript <- as.data.frame(VoxA[[2]])
...
```{r}
sentiment_transcript <- get_sentiment(transcript_cleaned, method="syuzhet") # sentiment per document
sentiment_whole <- get_sentiment(text_clean, method="syuzhet")

nrc_data <- get_nrc_sentiment(text) #nrc for text as a whole

TranscriptDatebySentiment <- transcript_cleaned %>%
  mutate(sentiment = sentiment_transcript)

TranscriptDatebySentimentfilter <- transcript_cleaned %>%
  mutate(sentiment = sentiment_transcript) %>%
...
```{r}
worstsentiment <- TranscriptDatebySentiment %>%
  filter(sentiment == min(sentiment)) %>%
  unnest_tokens(word, text) %>%
  anti_join(stop_words, by = "word")

bestsentiment <- TranscriptDatebySentiment %>%
  filter(sentiment == max(sentiment)) %>%
  unnest_tokens(word, text) %>%
  anti_join(stop_words, by = "word")

  bestsentiment_sen <- get_sentiment(bestsentiment$word, method="syuzhet")

  worstsentiment_sen <- get_sentiment(worstsentiment$word, method="syuzhet")
```

```{r}


ggplot(TranscriptDatebySentiment, aes(x = date, y = sentiment)) +
  geom_line() + 
  geom_point() +
  labs(title = "Sentiment over Time", x = "Date", y = "Sentiment Score") +
  geom_smooth()

ggplot(TranscriptDatebySentimentfilter, aes(x = date, y = sentiment)) +
  geom_line() + 
  geom_point() +
  labs(title = "Sentiment over Time", x = "Date", y = "Sentiment Score") +
  geom_smooth()
...
sentTest <- unlist(strsplit(transcript$text, "(?<=\\.)", perl = TRUE))
...
emotions_long <- prop.table(nrc_data[, 1:8]) %>%
  colSums() %>%
  sort() %>%
